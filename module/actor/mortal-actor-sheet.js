...
export class MortalActorSheet extends CoterieActorSheet {
  ...
  static get defaultOptions () {
    return mergeObject(super.defaultOptions, {
      classes: ['vm20DarkAge', 'sheet', 'actor', 'mortal'],
      template: 'systems/vm20DarkAge/templates/actor/mortal-sheet.html',
      width: 800,
      height: 700,
      tabs: [{
  ...
  /** @override */
  get template () {
    if (!game.user.isGM && this.actor.limited) return 'systems/vm20DarkAge/templates/actor/limited-sheet.html'
    return 'systems/vm20DarkAge/templates/actor/mortal-sheet.html'
  }
  ...
  getData () {
    ...
    data.sheetType = `${game.i18n.localize('vm20DarkAge.Mortal')}`
  ...
  _onRollDialog (event) {
    ...
    const template = `
      <form>
          <div class="form-group">
              <label>${game.i18n.localize('vm20DarkAge.SelectAbility')}</label>
              <select id="abilitySelect">${options}</select>
          </div>  
          <div class="form-group">
              <label>${game.i18n.localize('vm20DarkAge.Modifier')}</label>
              <input type="text" id="inputMod" value="0">
          </div>  
          <div class="form-group">
              <label>${game.i18n.localize('vm20DarkAge.Difficulty')}</label>
    ...
    buttons = {
      draw: {
        ...
        label: game.i18n.localize('vm20DarkAge.Roll'),
        callback: async (html) => {
      ...
      cancel: {
        ...
        label: game.i18n.localize('vm20DarkAge.Cancel')
      }
    }

    new Dialog({
      title: game.i18n.localize('vm20DarkAge.Rolling') + ` ${dataset.label}...`,
      content: template,
      buttons: buttons,
      default: 'draw'
    }).render(true)
